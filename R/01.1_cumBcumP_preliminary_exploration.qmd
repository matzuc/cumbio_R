...
es <- cumbio |> filter(model.model_number == 105)
...
mycurvePLOTS <- function(x, y, z, npoints, suff){
  ...
  # xx are the TLs
  xx <- NA;
  xx <- seq(1.6, 6,length.out = npoints)
  ...
  segY <- data.frame(y = seq(0, 1,length.out = npoints)) 
  segY$z <- predict(seg, newdata = segY, type = "response")
  ...
  A <- ggplot(df, aes(x, y)) +
  	geom_point() +
  	geom_line(lty = 3) +
  	geom_line(data = prS, col = "grey60", lty = 1) +
  	geom_point(aes(x = TLinfl, y = BIOinfl), size = 2.8, col = "green") +
  	geom_hline(yintercept = bpX, lty = 2, col = "red") +
  	ggtitle(suff) +
  	theme_few() +
  ...
  B <- ggplot(df, aes(y, z)) +
  	geom_point()+
  	geom_line(data = segY, col = "grey60", lty = 1) +
  	geom_line(lty = 3)+
  	geom_point(aes(x = bpX, y = bpY), size = 2.8, col = "red") +
  	geom_vline(xintercept = BIOinfl, lty = 2, col = "green") +
  	theme_few()+
  ...
  ggsave(here::here('out/plots', paste0(suff, '.png')), dpi = 400, height = 4.6, width = 9.5)
...
mycurve <- function(x, y, z, npoints, suff){
...
mycurve(ex$tl, ex$ycurv, ex$pcurv, 5000)
...
result <- dt[, mycurve(tl, ycurv, pcurv, 5000), by = .(model.model_name, model.model_number)]
result
...
table(cumbio$model.model_number)
table(cumbio$model.whole_food_web)

table(cumbio$model.ec)
filtered_data <- cumbio %>%
	filter(model.whole_food_web == "true") |>
...
```{r}
# Get unique groups
unique_groups <- unique(data.table(filtered_data)[, .( model.model_number)])
dt <- data.table(cumbio)
...
for(i in seq_len(nrow(unique_groups))){
  ...
  # Apply the function
  tryCatch({
    #result <- subset_dt[, mycurve(tl, ycurv, pcurv, model.model_name)]
  	suff2 <- paste0(subset_dt$model.model_name[1], "_", subset_dt$model.model_year[1])
    resT <- mycurvePLOTS(subset_dt$tl, subset_dt$ycurv, subset_dt$pcurv, 5000, suff2)
...
result <- dt[, mycurve(tl, ycurv, pcurv, 5000, suff), by = .(model.model_name, model.model_number)]
result
