...
```{r}
ex <- cumbio |> filter(area_name == "Italy (mainland)", year == 1980)
...
mycurve <- function(x, y){
  ...
  # xx are the TLs
  xx <- NA;
  xx <- seq(1.6, 5,length.out=10000)
  pr <- NA; length(pr) <- 10000# vector for prediction
...
long data frame for plotting the parameters time series
